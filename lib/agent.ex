defmodule Dantex.Agent do
  ...
  @type t :: %__MODULE__{
          model: Model.t(),
          tools: [Tool.t() | Tool.RemoteTool.t()],
          messages: [Message.t()],
  ...
  ## Options
    ...
    * `:tools` - List of tool modules or remote tools to use (default: [])
  ...
  ## Example
      ...
      Agent.new(
        ...
        tools: [WeatherTool, TimeTool, Dantex.Tool.RemoteTool.new(type: "web_search_20250305", name: "web_search")],
        sub_agents: %{
  ...
  """
  @spec add_tools(t(), [Tool.t() | Tool.RemoteTool.t()]) :: t()
  ...
  def run(%__MODULE__{} = agent, message) do
    ...
    run_conversation_loop(%{agent | messages: messages}, 0)
  end
  ...
  def chat_completion(%__MODULE__{} = agent) do
    case Model.chat_completion(agent.model, agent.messages, agent.tools, agent.timeout) do
  ...
  def step(%__MODULE__{} = agent) do
    case chat_completion(agent) do
      {:ok, {response_msg, _}, _} ->
        case agent.tool_adapter.extract_tool_calls(response_msg) do
          {:ok, processed_msg} ->
            updated_agent = add_message(agent, processed_msg)
  ...
  def execute_tools(%__MODULE__{messages: messages} = agent) when length(messages) > 0 do
    ...
    case last_message do
      %Message{tool_calls: tool_calls} when is_list(tool_calls) and length(tool_calls) > 0 ->
        case execute_tool_calls_and_update(agent, last_message) do
  ...
  defp run_conversation_loop(agent, iteration) do
    case step(agent) do
      {:ok, response_message, updated_agent} ->
        case handle_response_message(updated_agent, response_message) do
          {:continue, agent_with_tool_results} ->
            run_conversation_loop(agent_with_tool_results, iteration + 1)
          {:complete, final_message, final_agent} ->
            {:ok, final_message, final_agent}
          {:error, reason} ->
            {:error, reason}
        end
  ...
  @spec handle_response_message(t(), Message.t()) ::
          {:continue, t()} | {:complete, Message.t(), t()} | {:error, term()}
  defp handle_response_message(agent, %Message{tool_calls: tool_calls} = message)
       when is_list(tool_calls) and length(tool_calls) > 0 do
    case execute_tool_calls_and_update(agent, message) do
      {:ok, updated_agent} ->
        {:continue, updated_agent}
      {:error, reason} ->
        {:error, reason}
    end
  end

  defp handle_response_message(agent, message) do
    {:complete, message, agent}
  end

  @spec execute_tool_calls_and_update(t(), Message.t()) :: {:ok, t()} | {:error, term()}
  defp execute_tool_calls_and_update(agent, %Message{tool_calls: tool_calls} = message) do
    case validate_tool_call_retries(agent, tool_calls) do
      :ok ->
        execute_tools_and_add_results(agent, message)
      {:error, reason} ->
        error_msg = create_error_message("Max failed retries exceeded: #{reason}")
        {:ok, add_message(agent, error_msg)}
    end
  end

  @spec validate_tool_call_retries(t(), list()) :: :ok | {:error, String.t()}
  defp validate_tool_call_retries(%{max_failed_retries: nil}, _), do: :ok
  defp validate_tool_call_retries(_, []), do: :ok
  defp validate_tool_call_retries(agent, tool_calls) do
    case Enum.find(tool_calls, &exceeded_retry_limit?(agent, &1)) do
      nil -> :ok
      failed_call -> {:error, "Tool #{failed_call.function.name} exceeded retry limit"}
    end
  end

  @spec exceeded_retry_limit?(t(), Message.tool_call()) :: boolean()
  defp exceeded_retry_limit?(%{max_failed_retries: 0}, _), do: false
  defp exceeded_retry_limit?(agent, tool_call) do
    ...
    failed_count >= agent.max_failed_retries
  end

  @spec execute_tools_and_add_results(t(), Message.t()) :: {:ok, t()}
  defp execute_tools_and_add_results(agent, %Message{tool_calls: tool_calls}) do
    enhanced_context = Map.put(agent.context, :sub_agents, agent.sub_agents)
    tool_results = execute_tool_calls(agent.tools, tool_calls, enhanced_context)
    {tool_messages, original_results} = Enum.unzip(tool_results)

    updated_agent =
      agent
      |> add_messages(tool_messages)
      |> update_tool_history_with_results(tool_calls, tool_messages, original_results)

    {:ok, updated_agent}
  end

  @spec add_messages(t(), [Message.t()]) :: t()
  defp add_messages(agent, messages) do
    %{agent | messages: agent.messages ++ messages}
  end

  @spec update_tool_history_with_results(t(), list(), [Message.t()], list()) :: t()
  defp update_tool_history_with_results(agent, tool_calls, _tool_messages, original_results) do
    new_history_entries =
      Enum.zip(tool_calls, original_results)
    ...
    %{agent | tool_history: new_history_entries ++ agent.tool_history}
  end

  @spec create_error_message(String.t()) :: Message.t()
  defp create_error_message(error_text) do
    %Message{
      role: "assistant",
      content: error_text
    }
  ...
  @spec execute_tool_calls([Tool.t() | Tool.RemoteTool.t()], list(Message.tool_call()), map()) :: [{Message.t(), any()}]
    ...
    |> Enum.map(fn tool_call ->
      ...
      {result_message, original_result} =
        if tool == nil do
          ...
          # Execute local tool (remote tools don't generate tool_calls that reach here)
  ...
  # Helper function to find a local tool by name
  # Remote tools don't generate tool_calls so we only need to check local tools
  defp find_tool_by_name(tools, tool_name) do
    Enum.find(tools, &matches_tool_name?(&1, tool_name))
  end

  # Pattern match for remote tools - skip them
  defp matches_tool_name?(%Tool.RemoteTool{}, _tool_name), do: false

  # Pattern match for regular tools - check name
  defp matches_tool_name?(tool, tool_name), do: tool.tool_name() == tool_name
